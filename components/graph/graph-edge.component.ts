...
import { VtsGraphEdge } from './interface';

@Component({
  selector: '[vts-graph-edge]',
  template: `
    <ng-container
      *ngIf="customTemplate"
    ...
    <svg:g *ngIf="!customTemplate">
      <path class="vts-graph-edge-line" [attr.marker-end]="'url(#edge-end-arrow)'"></path>
      <svg:text class="vts-graph-edge-text" text-anchor="middle" dy="10" *ngIf="edge.label">
...
export class VtsGraphEdgeComponent implements OnInit, OnChanges {
  @Input() edge!: VtsGraphEdge;
