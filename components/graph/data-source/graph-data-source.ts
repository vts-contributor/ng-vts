...
import { VtsGraphDataDef } from '../interface';
import { VtsGraphBaseSource } from './base-graph-source';

export class VtsGraphData implements VtsGraphBaseSource<VtsGraphDataDef, string> {
  private _data = new BehaviorSubject<VtsGraphDataDef>({} as VtsGraphDataDef);
  dataSource!: VtsGraphDataDef;
  ...
  setData(data: VtsGraphDataDef): void {
    this.expansionModel?.clear();
    this.dataSource = data;
    this._data.next(data);
  }

  constructor(source?: VtsGraphDataDef) {
    if (source) {
      this.expansionModel?.clear();
  ...
  connect(): Observable<VtsGraphDataDef> {
