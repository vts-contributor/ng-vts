...
export class Minimap {
  ...
  private mainZoom: ZoomBehavior<VtsSafeAny, VtsSafeAny>;

  constructor(
    svg: SVGSVGElement,
    zoomG: SVGGElement,
    mainZoom: ZoomBehavior<VtsSafeAny, VtsSafeAny>,
    minimap: HTMLElement,
    maxWidth: number,
    labelPadding: number
  ) {
    ...
    const handleEvent = (event: VtsSafeAny): void => {
      ...
      const clickCoords = pointer(event, minimapSvgElement.node() as VtsSafeAny);
    ...
    viewpointElement.datum(this.viewpointCoord as VtsSafeAny).call(dragEvent as VtsSafeAny);
  ...
  private updateViewpoint(): void {
    ...
    select(this.svg).call(
      ...
      zoomIdentity.translate(mainX, mainY).scale(this.scaleMain)
    );
  }
  ...
  update(): void {
    ...
    for (const k of new Array(document.styleSheets.length).keys()) {
      try {
        const cssRules =
          (document.styleSheets[k] as VtsSafeAny).cssRules ||
          (document.styleSheets[k] as VtsSafeAny).rules;
        if (cssRules == null) {
          continue;
        }
    ...
    // viewpoint rect.
    select(this.minimapSvg).attr(this.minimapSize as VtsSafeAny);
    select(this.canvasBuffer).attr(this.minimapSize as VtsSafeAny);
   ...
   * @param transform
   */
  zoom(transform?: ZoomTransform | VtsZoomTransform): void {
    ...
    const w =
      Math.min(Math.max(0, x + viewpointWidth), mapWidth) - Math.min(Math.max(0, x), mapWidth);
    const h =
      Math.min(Math.max(0, y + viewpointHeight), mapHeight) - Math.min(Math.max(0, y), mapHeight);
