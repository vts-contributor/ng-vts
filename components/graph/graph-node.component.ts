...
import { VtsGraphGroupNode, VtsGraphNode } from './interface';
...
@Component({
  ...
  host: {
    ...
    '[class.vts-graph-node-expanded]': 'node.expanded',
    '[class.vts-graph-group-node]': 'node.type===0',
    '[class.vts-graph-base-node]': 'node.type===1',
    '(click)': 'triggerClick($event)'
  }
})
export class VtsGraphNodeComponent {
  @Input() node!: VtsGraphNode | VtsGraphGroupNode;
  @Input() @InputBoolean() noAnimation?: boolean;
  @Input() customTemplate?: TemplateRef<{
    $implicit: VtsGraphNode | VtsGraphGroupNode;
  }>;

  @Output() readonly nodeClick: EventEmitter<VtsGraphNode | VtsGraphGroupNode> = new EventEmitter();
  ...
  computeCXPositionOfNodeShape(): number {
    if ((this.node as VtsGraphGroupNode).expanded) {
      return this.node.x;
    }
