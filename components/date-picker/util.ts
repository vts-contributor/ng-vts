...
export const PREFIX_CLASS = 'vts-picker';

const defaultDisabledTime: DisabledTimeConfig = {
  vtsDisabledHours(): number[] {
    return [];
  },
  vtsDisabledMinutes(): number[] {
    return [];
  },
  vtsDisabledSeconds(): number[] {
    return [];
  }
};
...
export function isTimeValidByConfig(
  ...
  if (value) {
    ...
    const disabledHours = disabledTimeConfig.vtsDisabledHours();
    if (disabledHours.indexOf(hour) === -1) {
      const disabledMinutes = disabledTimeConfig.vtsDisabledMinutes(hour);
      if (disabledMinutes.indexOf(minutes) === -1) {
        const disabledSeconds = disabledTimeConfig.vtsDisabledSeconds(hour, minutes);
