...
import { valueFunctionProp } from '@ui-vts/ng-vts/core/util';

import { DateHelperService, VtsCalendarI18nInterface, VtsI18nService } from '@ui-vts/ng-vts/i18n';
...
export class DateTableComponent extends AbstractTable implements OnChanges, OnInit {
  ...
  makeHeadRow(): DateCell[] {
    ...
    for (let colIndex = 0; colIndex < this.MAX_COL; colIndex++) {
      ...
      weekDays.push({
        ...
        title: this.dateHelper.format(
          day.nativeDate,
          this.i18n.getDateFormat()?.DATE_TABLE_BODY_HEAD_TITLE || 'E'
        ), // eg. Tue
        content: this.dateHelper.format(
          day.nativeDate,
          this.i18n.getDateFormat()?.DATE_TABLE_BODY_HEAD_LABEL || this.getVeryShortWeekFormat()
        ), // eg. Tu,
        isSelected: false,
        isDisabled: false,
        onClick(): void {},
  ...
  makeBodyRows(): DateBodyRow[] {
    ...
    for (let week = 0; week < this.MAX_ROW; week++) {
      ...
      for (let day = 0; day < 7; day++) {
        ...
        const title = this.dateHelper.format(
          date.nativeDate,
          this.i18n.getDateFormat()?.DATE_TABLE_BODY_CONTENT_TITLE || dateFormat
        );
        const label = this.dateHelper.format(
          date.nativeDate,
          this.i18n.getDateFormat()?.DATE_TABLE_BODY_CONTENT_LABEL || 'dd'
        );
        const cell: DateCell = {
          trackByIndex: day,
      ...
      row.classMap = {
        [`vts-picker-week-panel-row`]: this.showWeek,
        [`vts-picker-week-panel-row-selected`]: this.showWeek && row.isActive
      };
      weekRows.push(row);
    }
    return weekRows;
  ...
  override getClassMap(cell: DateCell): { [key: string]: boolean } {
    ...
    return {
      ...
      [`vts-picker-cell-today`]: !!cell.isToday,
      [`vts-picker-cell-in-view`]: date.isSameMonth(this.activeDate)
    };
  }
}
