...
describe('VtsHighlightPipe', () => {
  let pipe: VtsHighlightPipe;

  beforeEach(() => {
    pipe = new VtsHighlightPipe();
  });

  it('should return highlight tag', () => {
    expect(pipe.transform('Ant Design of Angular', 'Angular')).toEqual(
      'Ant Design of <span>Angular</span>'
    );
    expect(pipe.transform('Ant Design of Angular', 'Ant')).toEqual(
      '<span>Ant</span> Design of Angular'
    );
  });
  ...
  it('should highlight when there is a new value', () => {
    ...
    expect(pipe.transform('ng-vts', 'ng')).toEqual('<span>ng</span>-zorro-antd');
  });
  ...
  it('should escapes regex keyword', () => {
    expect(
      ...
      'Escapes of characters <span>(\\, *, +, ?, |, {, [, (,), ^, $, ., #, and white space)</span>'
    );
  });
