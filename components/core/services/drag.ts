...
import { getEventPosition, isTouchEvent } from '@ui-vts/ng-vts/core/util';
...
export class VtsDragService {
  ...
  requestDraggingSequence(event: MouseEvent | TouchEvent): Observable<Delta> {
    ...
    return this.currentDraggingSequence.pipe(
      ...
      filter(
        (e: Delta) =>
          Math.abs(e.x) > this.draggingThreshold || Math.abs(e.y) > this.draggingThreshold
      ),
