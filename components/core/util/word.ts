...
/**
 * Splits `value` into an array of its words.
 * @example
 *
 * words('some, thing, & here')
 * => ['some', 'thing', 'here']
 *
 * words('some, thing, & here', /[^, ]+/g)
 * => ['some', 'thing', '&', 'here']
 */
export const words = (value: string, pattern?: RegExp) => {
  if (pattern === undefined) {
    const result = hasUnicodeWord(value)
      ? unicodeWords(value)
      : asciiWords(value);
    return result || [];
  }
