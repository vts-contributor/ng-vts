...
import { NgStyleInterface, NumberInput, VtsSafeAny } from '@ui-vts/ng-vts/core/types';
import { getStyleAsText, InputNumber, shallowEqual } from '@ui-vts/ng-vts/core/util';
...
const VTS_CONFIG_MODULE_NAME: VtsConfigKey = 'affix';
const VTS_AFFIX_CLS_PREFIX = 'vts-affix';
const VTS_AFFIX_DEFAULT_SCROLL_TIME = 20;
...
export class VtsAffixComponent implements AfterViewInit, OnChanges, OnDestroy, OnInit {
  readonly _vtsModuleName: VtsConfigKey = VTS_CONFIG_MODULE_NAME;
  static ngAcceptInputType_vtsOffsetTop: NumberInput;
  static ngAcceptInputType_vtsOffsetBottom: NumberInput;

  @ViewChild('fixedEl', { static: true })
  private fixedEl!: ElementRef<HTMLDivElement>;

  @Input() vtsTarget?: string | Element | Window;

  @Input()
  @WithConfig<number | null>()
  @InputNumber(undefined)
  vtsOffsetTop?: null | number;

  @Input()
  @WithConfig<number | null>()
  @InputNumber(undefined)
  vtsOffsetBottom?: null | number;

  @Output() readonly vtsChange = new EventEmitter<boolean>();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    const { vtsOffsetBottom, vtsOffsetTop, vtsTarget } = changes;

    if (vtsOffsetBottom || vtsOffsetTop) {
  ...
  private registerListeners(): void {
    ...
    this.positionChangeSubscription = this.ngZone.runOutsideAngular(() => {
      return merge(
        ...
        .pipe(auditTime(VTS_AFFIX_DEFAULT_SCROLL_TIME))
  ...
  private setAffixStyle(e: Event, affixStyle?: NgStyleInterface): void {
    ...
    if (fixed) {
      wrapEl.classList.add(VTS_AFFIX_CLS_PREFIX);
    } else {
      wrapEl.classList.remove(VTS_AFFIX_CLS_PREFIX);
    }
    this.updateRtlClass();
    if ((affixStyle && !originalAffixStyle) || (!affixStyle && originalAffixStyle)) {
      this.vtsChange.emit(fixed);
    }
  }
  ...
  updatePosition(e: Event): void {
    ...
    let offsetTop = this.vtsOffsetTop;
    ...
    if (typeof offsetTop !== 'number' && typeof this.vtsOffsetBottom !== 'number') {
      offsetMode.top = true;
      offsetTop = 0;
    } else {
      offsetMode.top = typeof offsetTop === 'number';
      offsetMode.bottom = typeof this.vtsOffsetBottom === 'number';
    }
    ...
    if (scrollTop >= elemOffset.top - (offsetTop as number) && offsetMode.top) {
      ...
      scrollTop <=
        elemOffset.top + elemSize.height + (this.vtsOffsetBottom as number) - targetInnerHeight &&
      offsetMode.bottom
    ) {
      ...
      this.setAffixStyle(e, {
        position: 'fixed',
        bottom: targetBottomOffset + (this.vtsOffsetBottom as number),
        left: targetRect.left + elemOffset.left,
        width
      });
      this.setPlaceholderStyle({
  ...
  private updateRtlClass(): void {
    const wrapEl = this.fixedEl.nativeElement;
    if (this.dir === 'rtl') {
      if (wrapEl.classList.contains(VTS_AFFIX_CLS_PREFIX)) {
        wrapEl.classList.add(`${VTS_AFFIX_CLS_PREFIX}-rtl`);
      } else {
        wrapEl.classList.remove(`${VTS_AFFIX_CLS_PREFIX}-rtl`);
      }
    } else {
      wrapEl.classList.remove(`${VTS_AFFIX_CLS_PREFIX}-rtl`);
    }
