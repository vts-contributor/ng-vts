...
import { VtsAffixModule } from './affix.module';
...
describe('affix', () => {
  let scrollService: VtsScrollService;
  let fixture: ComponentFixture<TestAffixComponent>;
  let context: TestAffixComponent;
  let debugElement: DebugElement;
  let component: VtsAffixComponent;
  let componentObject: VtsAffixPageObject;
  ...
  describe('resize', () => {
    ...
    it('should be reset placeholder size when container becomes greater', fakeAsync(() => {
      ...
      expect(componentObject.elementRef().style.width).toBe(
        `${componentObject.elementRef().offsetWidth}px`
      );
      setupInitialState();
      discardPeriodicTasks();
    }));
  ...
  describe('(vtsChange)', () => {
    ...
    beforeEach(() => {
      component.vtsChange.subscribe((returnValue: boolean) => {
  ...
  class VtsAffixPageObject {
    ...
    elementRef(): HTMLElement {
      return debugElement.query(By.css('vts-affix')).nativeElement;
    }
...
describe('affix-extra', () => {
  ...
  it('#getOffset', () => {
    const ret = fixture.componentInstance.vtsAffixComponent.getOffset(
  ...
  it('with window when scrolled below the bottom offset', fakeAsync(() => {
    ...
    fixture.detectChanges();
    const el = dl.query(By.css('vts-affix')).nativeElement as HTMLElement;
    ...
    tick(30);
    fixture.detectChanges();
    const ret = +(el.querySelector('.vts-affix') as HTMLElement).style.bottom!.replace('px', '');
...
describe('affix RTL', () => {
  ...
  it('should className correct on dir change', fakeAsync(() => {
    ...
    tick(30);
    fixture.detectChanges();
    expect(el.querySelector('.vts-affix')?.classList).toContain('vts-affix-rtl');
  }));
});
@Component({
  template: `
    <vts-affix
      id="affix"
      [vtsTarget]="fakeTarget"
      [vtsOffsetTop]="newOffset"
      [vtsOffsetBottom]="newOffsetBottom"
    >
...
class TestAffixComponent {
  @ViewChild(VtsAffixComponent, { static: true })
  vtsAffixComponent!: VtsAffixComponent;
