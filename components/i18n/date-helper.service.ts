...
import { WeekDayIndex, ɵNgTimeParser } from '@ui-vts/ng-vts/core/time';
import { mergeDateConfig, VtsDateConfig, VTS_DATE_CONFIG } from './date-config';
import { VtsI18nService } from './vts-i18n.service';

export function DATE_HELPER_SERVICE_FACTORY(
  injector: Injector,
  config: VtsDateConfig
): DateHelperService {
  const i18n = injector.get(VtsI18nService);
  return new DateHelperByDateFns(i18n, config);
...
@Injectable({
  ...
  deps: [Injector, [new Optional(), VTS_DATE_CONFIG]]
})
export abstract class DateHelperService {
  constructor(
    protected i18n: VtsI18nService,
    @Optional() @Inject(VTS_DATE_CONFIG) protected config: VtsDateConfig
  ) {
