import { ByteUnit, VtsBytesPipe } from './vts-bytes.pipe';

describe('VtsBytesPipe', () => {
  let pipe: VtsBytesPipe;

  beforeEach(() => {
    pipe = new VtsBytesPipe();
  });
  ...
  it('Should return 100 TB', () => {
    const bytes = 100;
    const unit = 'TB';
    const result = VtsBytesPipe.formatResult(bytes, unit);
  ...
  it('Should return 1', () => {
    ...
    const result = VtsBytesPipe.calculateResult(format, bytes);
