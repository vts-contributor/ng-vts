...
import { getElementOffset, isNotNil } from '@ui-vts/ng-vts/core/util';
import { Subject } from 'rxjs';
import { takeUntil } from 'rxjs/operators';
import {
  FADE_CLASS_NAME_MAP,
  MODAL_MASK_CLASS_NAME,
  VTS_CONFIG_MODULE_NAME,
  ZOOM_CLASS_NAME_MAP
} from './modal-config';
...
export function throwVtsModalContentAlreadyAttachedError(): never {
...
export class BaseModalContainerComponent extends BasePortalOutlet implements OnDestroy {
  ...
  get showMask(): boolean {
    ...
    return !!getValueWithConfig<boolean>(this.config.vtsMask, defaultConfig.vtsMask, true);
  }
  ...
  get maskClosable(): boolean {
    ...
    return !!getValueWithConfig<boolean>(
      this.config.vtsMaskClosable,
      defaultConfig.vtsMaskClosable,
      true
    );
  }
  ...
  constructor(
    ...
    this.isStringContent = typeof config.vtsContent === 'string';
  ...
  attachComponentPortal<T>(portal: ComponentPortal<T>): ComponentRef<T> {
    if (this.portalOutlet.hasAttached()) {
      throwVtsModalContentAlreadyAttachedError();
    }
  ...
  private animationDisabled(): boolean {
    return this.config.vtsNoAnimation || this.animationType === 'NoopAnimations';
  }
  ...
  private setZIndexForBackdrop(): void {
    const backdropElement = this.overlayRef.backdropElement;
    if (backdropElement) {
      if (isNotNil(this.config.vtsZIndex)) {
        this.render.setStyle(backdropElement, 'z-index', this.config.vtsZIndex);
      }
    }
  }
  ...
  bindBackdropStyle(): void {
    ...
    if (backdropElement) {
      ...
      if (
        typeof this.config.vtsMaskStyle === 'object' &&
        Object.keys(this.config.vtsMaskStyle).length
      ) {
        const styles: { [key: string]: string } = {
          ...this.config.vtsMaskStyle
        };
        Object.keys(styles).forEach(key => {
