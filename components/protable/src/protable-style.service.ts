 ...
 */
 ...
 @Injectable()
 export class VtsProTableStyleService {
   theadTemplate$ = new ReplaySubject<TemplateRef<VtsSafeAny>>(1);
   ...
   noResult$ = new ReplaySubject<string | TemplateRef<VtsSafeAny> | undefined>(1);
   ...
   ]).pipe(map(([widthConfig, listOfWidth]) => (widthConfig.length ? widthConfig : listOfWidth)));
   ...
   setTheadTemplate(template: TemplateRef<VtsSafeAny>): void {
   ...
   setListOfTh(listOfTh: ReadonlyArray<VtsThMeasureDirective>): void {
     let columnCount = 0;
     listOfTh.forEach(th => {
       columnCount += (th.colspan && +th.colspan) || (th.colSpan && +th.colSpan) || 1;
     });
     const listOfThPx = listOfTh.map(item => item.vtsWidth);
     this.columnCount$.next(columnCount);
     this.listOfThWidthConfigPx$.next(listOfThPx);
   }
 
   setListOfMeasureColumn(listOfTh: ReadonlyArray<VtsThMeasureDirective>): void {
   ...
   setNoResult(noResult: string | TemplateRef<VtsSafeAny> | undefined): void {
