...
@Component({
  ...
  selector: 'vts-slider-handle',
  exportAs: 'vtsSliderHandle',
  preserveWhitespaces: false,
  template: `
    <div
      #handle
      class="vts-slider-handle"
      tabindex="0"
      vts-tooltip
      [ngStyle]="style"
      [vtsTooltipTitle]="
        tooltipFormatter === null || tooltipVisible === 'never' ? null : tooltipTitle
      "
      [vtsTooltipTrigger]="null"
      [vtsTooltipPlacement]="tooltipPlacement"
    ></div>
  `,
  host: {
...
export class VtsSliderHandleComponent implements OnChanges {
  ...
  @Input() offset?: number;
  @Input() value?: number;
  @Input() tooltipVisible: VtsSliderShowTooltip = 'default';
  ...
  constructor(private sliderService: VtsSliderService, private cdr: ChangeDetectorRef) {}
    ...
    const positionStyle = vertical
      ...
      : {
          ...
          transform: `translateX(${
            reverse ? (this.dir === 'rtl' ? '-' : '+') : this.dir === 'rtl' ? '+' : '-'
          }50%)`
        };
