...
@Component({
  selector: '[vts-upload-btn]',
  exportAs: 'vtsUploadBtn',
  templateUrl: './upload-btn.component.html',
  host: {
    '[attr.tabindex]': '"0"',
    '[attr.role]': '"button"',
    '[class.vts-upload-disabled]': 'options.disabled',
    '(click)': 'onClick()',
...
export class VtsUploadBtnComponent implements OnDestroy {
  ...
  private traverseFileTree(files: DataTransferItemList): void {
    const _traverseFileTree = (item: VtsSafeAny, path: string) => {
      if (item.isFile) {
        item.file((file: File) => {
        ...
        dirReader.readEntries((entries: VtsSafeAny) => {
    ...
    for (const file of files as VtsSafeAny) {
  ...
  private attachUid(file: VtsUploadFile): VtsUploadFile {
    if (!file.uid) {
  ...
  uploadFiles(fileList: FileList | File[]): void {
    ...
    filters$.subscribe(
      list => {
        list.forEach((file: VtsUploadFile) => {
          this.attachUid(file);
  ...
  private post(file: VtsUploadFile): void {
    if (this.destroy) {
      return;
    }
    let process$: Observable<string | Blob | File | VtsUploadFile> = of(file);
    const opt = this.options;
    const { uid } = file;
    const { action, data, headers, transformFile } = opt;

    const args: VtsUploadXHRArgs = {
    ...
    if (typeof action === 'function') {
      const actionResult = (action as (file: VtsUploadFile) => string | Observable<string>)(file);
    ...
    if (typeof data === 'function') {
      const dataResult = (data as (file: VtsUploadFile) => {} | Observable<{}>)(file);
    ...
    if (typeof headers === 'function') {
      const headersResult = (headers as (file: VtsUploadFile) => {} | Observable<{}>)(file);
    ...
    process$.subscribe(newFile => {
      ...
      if (!(req$ instanceof Subscription)) {
        warn(`Must return Subscription type in '[vtsCustomRequest]' property`);
      }
      this.reqs[uid] = req$;
      opt.onStart!(file);
    });
  ...
  private xhr(args: VtsUploadXHRArgs): Subscription {
    ...
    formData.append(args.name!, args.postFile as VtsSafeAny);
    ...
    return this.http.request(req).subscribe(
      (event: HttpEvent<VtsSafeAny>) => {
        if (event.type === HttpEventType.UploadProgress) {
          if (event.total! > 0) {
            (event as VtsSafeAny).percent = (event.loaded / event.total!) * 100;
          }
