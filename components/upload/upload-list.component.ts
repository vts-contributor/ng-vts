...
interface UploadListFile extends VtsUploadFile {
  isImageUrl?: boolean;
  isUploading?: boolean;
...
@Component({
  ...
  host: {
    '[class.vts-upload-list-rtl]': `dir === 'rtl'`,
    '[class.vts-upload-list-text]': `listType === 'text'`,
    '[class.vts-upload-list-picture]': `listType === 'picture'`,
    '[class.vts-upload-list-picture-card]': `listType === 'picture-card'`
  },
  preserveWhitespaces: false,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class VtsUploadListComponent implements OnChanges {
  list: UploadListFile[] = [];
  ...
  @Input() listType!: VtsUploadListType;
  @Input()
  set items(list: VtsUploadFile[]) {
    this.list = list;
  }
  @Input() icons!: VtsShowUploadList;
  @Input() onPreview?: (file: VtsUploadFile) => void;
  ...
  isImageUrl(file: VtsUploadFile): boolean {
  ...
  private genThumb(): void {
    ...
    const win = window as VtsSafeAny;
    if (
      !this.showPic ||
    ...
    this.list
      ...
      .forEach(file => {
        file.thumbUrl = '';
        (this.previewFile
          ? this.previewFile(file).toPromise()
          : this.previewImage(file.originFileObj!)
        ).then(dataUrl => {
  ...
  private showDownload(file: VtsUploadFile): boolean {
  ...
  handlePreview(file: VtsUploadFile, e: Event): void {
    if (!this.onPreview) {
      return;
    }
