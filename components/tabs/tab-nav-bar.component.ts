...
import { VtsTabPositionMode, VtsTabScrollEvent, VtsTabScrollListOffsetEvent } from './interfaces';
...
@Component({
  ...
  template: `
    <div
      class="vts-tabs-nav-wrap"
      [class.vts-tabs-nav-wrap-ping-left]="pingLeft"
      [class.vts-tabs-nav-wrap-ping-right]="pingRight"
      [class.vts-tabs-nav-wrap-ping-top]="pingTop"
      [class.vts-tabs-nav-wrap-ping-bottom]="pingBottom"
      #navWarp
    >
      <div
        class="vts-tabs-nav-list"
        #navList
        vtsTabScrollList
    ...
    <vts-tab-nav-operation
      ...
      [items]="hiddenItems"
    ></vts-tab-nav-operation>
    <div class="vts-tabs-extra-content" *ngIf="extraTemplate">
...
export class VtsTabNavBarComponent
  implements OnInit, AfterViewInit, AfterContentChecked, OnDestroy, OnChanges
{
  ...
  @Output() readonly tabScroll = new EventEmitter<VtsTabScrollEvent>();

  @Input() position: VtsTabPositionMode = 'horizontal';
  ...
  @ViewChild(VtsTabNavOperationComponent, { static: true })
  operationRef!: VtsTabNavOperationComponent;
  @ViewChild(VtsTabAddButtonComponent, { static: false })
  addBtnRef!: VtsTabAddButtonComponent;
  @ViewChild(VtsTabsInkBarDirective, { static: true })
  inkBar!: VtsTabsInkBarDirective;
  @ContentChildren(VtsTabNavItemDirective, { descendants: true })
  items!: QueryList<VtsTabNavItemDirective>;
  ...
  pingTop = false;
  pingBottom = false;
  hiddenItems: VtsTabNavItemDirective[] = [];

  private keyManager!: FocusKeyManager<VtsTabNavItemDirective>;
  ...
  ngAfterViewInit(): void {
    ...
    this.keyManager = new FocusKeyManager<VtsTabNavItemDirective>(this.items)
    ...
    merge(
      this.vtsResizeObserver.observe(this.navWarpRef),
      this.vtsResizeObserver.observe(this.navListRef)
    )
  ...
  onSelectedFromMenu(tab: VtsTabNavItemDirective): void {
  ...
  onOffsetChange(e: VtsTabScrollListOffsetEvent): void {
  ...
  private scrollToTab(tab: VtsTabNavItemDirective): void {
  ...
  private alignInkBarToSelectedTab(): void {
    const selectedItem =
      this.items && this.items.length ? this.items.toArray()[this.selectedIndex] : null;
    const selectedItemElement = selectedItem ? selectedItem.elementRef.nativeElement : null;

    if (selectedItemElement) {
      /**
       * .vts-tabs-nav-list - Target offset parent element
       *   └──.vts-tabs-tab
       *        └──.vts-tabs-tab-btn - Currently focused element
       */
  ...
  private setPingStatus(): void {
    ...
    (Object.keys(ping) as Array<'top' | 'right' | 'bottom' | 'left'>).forEach(pos => {
      const className = `vts-tabs-nav-wrap-ping-${pos}`;
      if (ping[pos]) {
