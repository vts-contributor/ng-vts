...
import {
  VtsTabScrollEvent,
  VtsTabScrollEventHandlerFun,
  VtsTabScrollListOffset,
  VtsTabScrollListOffsetEvent
} from './interfaces';
...
@Directive({
  selector: '[vtsTabScrollList]'
})
export class VtsTabScrollListDirective implements OnInit, OnDestroy {
  ...
  touchPosition: VtsTabScrollListOffset | null = null;
  lastOffset: VtsTabScrollListOffset | null = null;
  motion = -1;

  unsubscribe: () => void = () => void 0;

  @Output()
  readonly offsetChange = new EventEmitter<VtsTabScrollListOffsetEvent>();
  ...
  subscribeWrap<T extends VtsTabScrollEvent['event']>(
    type: VtsTabScrollEvent['type'],
    observable: Observable<T>,
    handler: VtsTabScrollEventHandlerFun<T>
  ): Subscription {
    return observable.subscribe(event => {
      this.tabScroll.emit({
        type,
        event
      } as VtsTabScrollEvent);
