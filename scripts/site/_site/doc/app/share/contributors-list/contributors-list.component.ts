...
@Component({
  selector: 'vts-contributors-list',
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <ul
      class="contributors-list"
      style="display: flex; list-style: none; margin: 0px; padding: 0px;"
    >
      <a
        *ngFor="let item of list"
        vts-tooltip
        [vtsTooltipTitle]="(language === 'en' ? 'Contributors: ' : '文档贡献者: ') + item.name"
        [attr.href]="item.url"
        target="_blank"
      >
        <vts-avatar [vtsText]="item.name" [vtsSrc]="item.avatar"></vts-avatar>
      </a>
    </ul>
  `
})
export class VtsContributorsListComponent implements OnInit, OnDestroy {
  language = 'en';
  ...
  getContributors(path: string): void {
    this.http
      .get(`https://api.github.com/repos/NG-ZORRO/ng-vts/commits`, {
        params: {
          path
        }
      })
