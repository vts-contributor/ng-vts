...
module.exports = function generateDocs(rootPath, docsMap) {
  ...
  for (const name in docsMap) {
    ...
    generateTemplate(docsPath, name, null, en);
...
function generateToc(meta, raw) {
  ...
  for (let i = 0; i < ast.children.length; i++) {
    ...
    if (child.type === 'heading' && child.depth === 2) {
      ...
      links += `<vts-link vtsHref="#${lowerText}" vtsTitle="${text}"></vts-link>`
    }
  }
  return `
<vts-affix class="toc-affix" [vtsOffsetTop]="86">
    <vts-anchor [vtsOffsetTop]="86" [vtsAffix]="false" vtsShowInkInFixed (vtsClick)="goLink($event)">
        ${links}
    </vts-anchor>
</vts-affix>`;
}
...
function generateModule(docsPath, docsMap) {
  ...
  let declarations = '';
  for (const name in docsMap) {
    const componentName = `VtsDoc${capitalizeFirstLetter(camelCase(name))}`;
