...
// read components folder
  ...
  rootDir.filter(i => i !== 'lib').forEach(componentName => {
    ...
    if (componentName === 'style' || componentName === 'core' || componentName === 'locale' || componentName === 'i18n' || componentName === 'version' || componentName === 'cdk') {
      return;
    }
      ...
      if (fs.existsSync(demoDirPath)) {
        ...
        demoDir.forEach(demo => {
          if (/.md$/.test(demo)) {
            ...
            demoMap[nameKey]['name'] = `VtsDemo${camelCase(capitalizeFirstLetter(componentName))}${camelCase(capitalizeFirstLetter(nameKey))}Component`;
        ...
        demoDir.forEach(demo => {
          if (/.js$/.test(demo)) {
            const nameKey = nameWithoutSuffixUtil(demo);
            demoMap[nameKey].js = String(fs.readFileSync(path.join(demoDirPath, demo)));
            fs.writeFileSync(path.join(showCaseComponentPath, demo), demoMap[nameKey].js);
          }
        })
    }
    ...
    delete result.docZh
    componentsDocMap[componentName] = { en: result.docEn.meta };
